...
MIN_LIFT = 1.0
COLLECTOR_THEME_BIAS = 0.005
COLLECTOR_THEME_KEYWORDS = ("card", "graded", "elite", "booster")
PROMO_HIGH_LIFT_THRESHOLD = 1.5
PROMO_HIGH_DISCOUNT       = 50   
PROMO_LOW_DISCOUNT        = 20   
ITERATION_CUTOFF_FRACTIONS = [0.25, 0.50, 1.00]
ITERATION_NAMES = [
    "Iteration 1: Early Launch Phase",
    "Iteration 2: Mid-Season Update",
    "Iteration 3: Full Holiday Trend",
]
TOP_N_RULES = 3
...
def clean_transactions(df: pd.DataFrame) -> pd.DataFrame:
    ...
    print(
        ...
        f" | nulls removed: {original_count - after_nulls}"
        f" | duplicates removed: {after_nulls - after_dupes}"
        f" | {len(df)} rows out"
    )
    return df
def build_iteration_ranges(max_transaction_id: int) -> list:
    """Build cumulative iteration windows from ITERATION_CUTOFF_FRACTIONS."""
    ...
    ranges = []
    for name, fraction in zip(ITERATION_NAMES, ITERATION_CUTOFF_FRACTIONS):
        cutoff = max(1, round(max_transaction_id * fraction))
        ranges.append((name, 1, cutoff))
    return ranges
...
def is_collector_theme_rule(antecedents, consequents) -> bool:
    ...
    return any(
        ...
        for kw in COLLECTOR_THEME_KEYWORDS
    )
...
def pick_canonical_direction(rules: pd.DataFrame, item_support: dict) -> pd.DataFrame:
    """For mirror rule pairs, keep only the direction where the rarer item is
    the antecedent (higher-support item predicts lower-support item)."""
    kept_rows = []
    seen_symmetric = set()
...
def compute_score(rules: pd.DataFrame) -> pd.DataFrame:
    """Apply the weighted composite score using the config weights."""
    rules["score"] = (
          (rules["support"]    * WEIGHT_SUPPORT)
        + (rules["confidence"] * WEIGHT_CONFIDENCE)
        + (rules["lift"]       * WEIGHT_LIFT)
    )
    return rules
def _empty_result(
...
def analyze_iteration(
    ...
    if total_transactions <= THRESHOLD_SMALL:
        min_support = SUPPORT_SMALL
    elif total_transactions <= THRESHOLD_MEDIUM:
        min_support = SUPPORT_MEDIUM
    else:
        min_support = SUPPORT_LARGE
    print(f"  Transactions: {total_transactions} | Auto-Support: {min_support}")
    frequent_itemsets = fpgrowth(basket, min_support=min_support, use_colnames=True)
    if frequent_itemsets.empty:
        print("  No frequent itemsets found.")
        return _empty_result(iteration_name, min_id, max_id, total_transactions, raw_row_count, min_support)
    ...
    rules["rankingScore"] = (
        rules["score"]
        + rules["collectorThemeEligible"].astype(float) * COLLECTOR_THEME_BIAS
    )
    top_rules = (
        rules
        .sort_values(
    ...
    if not stable_itemsets.empty:
        ...
        if not stable_rules_df.empty:
            stable_rules_df = compute_score(stable_rules_df)
            stable_item_support = {
                list(row["itemsets"])[0]: float(row["support"])
                for _, row in stable_itemsets.iterrows()
                if len(row["itemsets"]) == 1
            }
            stable_rules_df = pick_canonical_direction(stable_rules_df, stable_item_support)
    ...
    for rank, row in top_rules.iterrows():
        ...
        promo_discount = PROMO_HIGH_DISCOUNT if lift_value > PROMO_HIGH_LIFT_THRESHOLD else PROMO_LOW_DISCOUNT
...
def main() -> None:
    ...
    print("=" * 60)
    print(f"  Scoring weights     : support={WEIGHT_SUPPORT} | confidence={WEIGHT_CONFIDENCE} | lift={WEIGHT_LIFT}")
    print(f"  Support thresholds  : small={SUPPORT_SMALL} (≤{THRESHOLD_SMALL} TX) | medium={SUPPORT_MEDIUM} (≤{THRESHOLD_MEDIUM} TX) | large={SUPPORT_LARGE}")
    print(f"  Promo tiers         : lift>{PROMO_HIGH_LIFT_THRESHOLD} → {PROMO_HIGH_DISCOUNT}% off | otherwise → {PROMO_LOW_DISCOUNT}% off")
    print(f"  Collector bias      : {COLLECTOR_THEME_BIAS}")
    print(f"  Top N rules         : {TOP_N_RULES}")
    print(f"\nLoading transactions from: {DATA_FILE}")
    ...
    output = {
        ...
        "engineConfig": {
            "weights":             {"support": WEIGHT_SUPPORT, "confidence": WEIGHT_CONFIDENCE, "lift": WEIGHT_LIFT},
            "supportThresholds":   {"small": SUPPORT_SMALL, "medium": SUPPORT_MEDIUM, "large": SUPPORT_LARGE},
            "sizeThresholds":      {"small": THRESHOLD_SMALL, "medium": THRESHOLD_MEDIUM},
            "minLift":             MIN_LIFT,
            "collectorBias":       COLLECTOR_THEME_BIAS,
            "promoHighLift":       PROMO_HIGH_LIFT_THRESHOLD,
            "promoHighDiscount":   PROMO_HIGH_DISCOUNT,
            "promoLowDiscount":    PROMO_LOW_DISCOUNT,
            "topNRules":           TOP_N_RULES,
        },
