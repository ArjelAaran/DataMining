...
export function BusinessActionCenter({ iteration, analysis, loading }: BusinessActionCenterProps) {
  ...
  const handleGenerateCampaign = () => {
    ...
    setCampaign({
      ...
      lift:            rule.lift,
      code:            rule.promoCode,
      discountPercent: rule.promoDiscount,
    });
  };
  const engineConfig = (analysis as any)?.engineConfig ?? null;
  return (
      ...
      {engineConfig && (
        <div className="border border-gray-800 bg-gray-950">
          <div className="border-b border-gray-800 px-4 py-2 flex items-center gap-3">
            ...
            <span className="text-[10px] tracking-[0.25em] uppercase text-gray-500 font-bold">Engine Config Snapshot</span>
            <span className="text-[10px] text-gray-700 tracking-wider ml-2">— settings that produced this run</span>
          </div>
          <div className="px-4 py-3 grid grid-cols-2 md:grid-cols-4 gap-4 text-[10px] text-gray-600 tracking-wider">
            <div>
              <div className="uppercase mb-1">Scoring Weights</div>
              <div className="text-gray-400">
                Supp {engineConfig.weights.support} · Conf {engineConfig.weights.confidence} · Lift {engineConfig.weights.lift}
              </div>
            </div>
            <div>
              <div className="uppercase mb-1">Promo Tiers</div>
              <div className="text-gray-400">
                Lift &gt;{engineConfig.promoHighLift} → <span className="text-red-400">{engineConfig.promoHighDiscount}% off</span>
                {' '}· else <span className="text-gray-300">{engineConfig.promoLowDiscount}% off</span>
              </div>
            </div>
            <div>
              <div className="uppercase mb-1">Support Thresholds</div>
              <div className="text-gray-400">
                {engineConfig.supportThresholds.small} / {engineConfig.supportThresholds.medium} / {engineConfig.supportThresholds.large}
              </div>
            </div>
            <div>
              <div className="uppercase mb-1">Collector Bias</div>
              <div className="text-gray-400">+{engineConfig.collectorBias} to TCG rules</div>
            </div>
          </div>
        </div>
      )}
